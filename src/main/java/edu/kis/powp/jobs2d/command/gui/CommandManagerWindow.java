...
public class CommandManagerWindow extends JFrame implements WindowComponent {
	...
	public CommandManagerWindow(ICommandManager commandManager) {
		...
		commandPreviewDriver = new ScaledLineDriverAdapter(iconDraw, LineFactoryWithThinLine.getBasicThinLine(),
				"basic").setScale(0.25);
	...
	public void updateCurrentCommandStatsField() {
		...
		if (currentCommand != null) {
			...
			currentCommandStatsField
					.append("Operations length: " + Math.round(countingVisitor.getTotalLength() * 100) / 100.0 + "\n");
			currentCommandStatsField.append(
					"OperateTo length: " + Math.round(countingVisitor.getOperateToLength() * 100) / 100.0 + "\n");
			currentCommandStatsField.append("Operation time: ");
			if (countingVisitor.getVisitTime() == null) {
				currentCommandStatsField.append("\n");
			} else {
				currentCommandStatsField.append(
						countingVisitor.getVisitTime().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))
								+ "\n");
			}
		}
